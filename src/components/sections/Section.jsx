  ...
  return (
    <section className={cn('space-y-4 rounded-3xl bg-card/40 p-4 shadow-sm ring-1 ring-border/40', className)}>
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          {title && <h2 className="text-xl font-semibold text-foreground md:text-2xl">{title}</h2>}
      ...
      {children ?? (
        <BooksMasonry books={books} isLoading={isLoading} onAddToCart={onAddToCart} onOpen={onOpen} />
      )}
    </section>
  );
}

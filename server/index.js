...
const getModeratorFromRequest = async (req) => {
  ...
  try {
    ...
    pushRole(userMetadata.role);
    pushRole(appMetadata.role);
    ...
    const collectRoles = (value) => {
      ...
      if (typeof value === 'object') {
        Object.entries(value).forEach(([key, isEnabled]) => {
          if (isEnabled) {
            pushRole(key);
          }
        });
    ...
    collectRoles(profile.roles);
    collectRoles(userMetadata.roles);
    collectRoles(appMetadata.roles);
    collectRoles(appMetadata.claims);

    const appClaims = (appMetadata && typeof appMetadata === 'object' && appMetadata.claims) || {};
    collectRoles(appClaims.roles);

    const hasAdminFlag = Boolean(
      profile.is_admin ||
        userMetadata.is_admin ||
        appMetadata.is_admin ||
        appMetadata.claims_admin ||
        appClaims.admin ||
        appClaims.is_admin
    );
    const hasModeratorFlag = Boolean(
      profile.is_moderator ||
        userMetadata.is_moderator ||
        appMetadata.is_moderator ||
        appMetadata.claims_moderator ||
        appClaims.moderator ||
        appClaims.is_moderator
    );
